(ns rms_underwriting.main
  "Kotodama WASM entrypoint for the rms_underwriting clean-room actor (L4) — Clojure port.
  ...
  facts (`rms_underwriting.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rms_underwriting")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "rmsunder_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "rmsunder_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"rms_underwriting.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rms_underwriting-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rms_underwriting-compat/ready)
