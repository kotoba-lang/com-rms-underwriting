(ns rms_underwriting.main-test
  "Contract + behavioral test for the rms_underwriting-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "rms_underwriting-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'rms_underwriting.main-test)]
